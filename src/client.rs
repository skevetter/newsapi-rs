...
impl NewsApiClient<reqwest::Client> {
    ...
    pub async fn get_everything(
        ...
        let mut url = self.base_url.clone();
        <NewsApiClient<reqwest::Client>>::get_endpoint_with_query_params_for_everything(
            &mut url, request,
        );
        log::debug!("Request URL: {}", url.as_str());
    ...
    pub async fn get_top_headlines(
        ...
        log::debug!("Request: {:?}", request);
        <NewsApiClient<reqwest::Client>>::top_headlines_validate_request(request)?;

        let mut url = self.base_url.clone();
        <NewsApiClient<reqwest::Client>>::get_endpoint_with_query_params_for_top_headlines(
            &mut url, request,
        );
        log::debug!("Request URL: {}", url.as_str());
